...
window.jQuery = function (selectorOrArrayOrTemplate) {
  ...
  return {
    ...
    find: function find(selector) {
      ...
      return jQuery(array); //const newApi = jQuery(array)
      // return newApi  
      //  可以简写成： return jQuery
    },
    end: function end() {
    ...
    children: function children() {
      var array = [];
      this.each(function (node) {
        array.push.apply(array, _toConsumableArray(node.children)); //展开操作符，展开获取的children数组
      });
      return this.jQuery(array);
    },
    ...
    addClass: function addClass(className) {
      for (var i = 0; i < elements.length; i++) {
        ...
        this.Element.classList.add(className); //elements的classList添加className
      }

      return this;
    },
    oldApi: selectorOrArrayOrTemplate.oldApi //把后面的属性一个一个复制给api/让api.__proto__指向$.fn    

  };
};

window.$ = window.jQuery; //get/each/find/print(打印)...这些都是key，{}里面的是value，api是对象。
//api会对应多个对象
